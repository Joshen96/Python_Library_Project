...
class BookPage(tk.Frame):  #### 전체 도서 페이지 
    ...
    def bookEdit(self, title, author, publisher, price, link, ISBN, photo, bookdescription):
        ...
        btn_cancel = Button(self.window, text="취소", bg="#0099ff", fg="white", command = lambda :cancel() )
        ...
        if key == 'info':
            ...
            try: # 도서 클릭안하고 도서수정 버튼 눌렀을 경우 예외처리
                ...
                if treeviewValues[4] =='대여불가':
                    messagebox.showinfo("알림", "대여중이거나 삭제되었습니다.")
                else:
                    for k in df1_list:
...
class Userpage(tk.Frame):   #### 전체 회원 페이지 ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    def User_add_save(self, phone_number, name, birth_year, birth_month, birth_day, gender, email, photo): ### 유저등록 여기부터 시작 
        ...
        if self.User_add_nameEntry.get() == '':
            ...
            if messagebox.askyesno("등록", self.addtext + "정말 등록하시겠습니까? "):
                ...
                num = int(len(df1))
                print(num)
                df1.loc[num] = [phone_number.get(), self.User_add_nameEntry.get(), birth, str(gender.get()), email.get(), date.today(), " ", '0', img]
    ...
    def useradd(self):
        ...
        self.User_add_manBtn=Radiobutton(self.User_add_sexFrame, text='남자', value = '1', variable = sexcheck) # 남자 체크시 sexcheck에 0 저장
    ...
    def userinfo(self): # 트리뷰 클릭한 값 넘기기
        try: # 도서 클릭안하고 도서정보 버튼 눌렀을 경우 예외처리
            ...
            for k in df2_list:
                print(k)
                print(k[0])
                if str(treeviewValues[3]) == str(k[0])  :
                    gen = self.gender1(str(k[3]))
...
class RentBook(tk.Frame):  #### 대여하기 (도서) 선택 페이지
    ...
    def __init__(self, master, name): ## 선택회원 이름을 가지고옴
        ...
        select_book_btn=ttk.Button(self,text='도서 대여',command=lambda:[self.user_rent_book(name),self.destroy(),self.successRent()])  # 함수추가 하기
