...
class RentPage(tk.Frame):  #### 대여하기 (이름)선택 페이지
    ...
    def __init__(self, master):
        ...
        #이름 검색 버튼 
        search_name_btn=ttk.Button(frame_r,text='검색',command="")       #lambda : self.RentuserSearch(rent_name_Entry.get())) # 함수 커맨트 빈칸
...
#####################################################################
    ...
    def RentuserSearch(self, name):  # 대여회원 검색
        self.name= name
        print(name)
        df2 = pd.read_csv ('user.csv') ## 회원정보 받아오기
        ...
        searchText = self.name # 검색창 값 가져오기
        print(searchText)
        ...
        for e in df2_rent_can_user_list:
            if self.name in e[1]: # 이름과 부분일치할 경우 
                gen = self.gender1(int(e[3]))
                cUser = self.currentUser(e[6])
                # 표에 데이터 삽입
