...
def bookInfo(title, author, publisher, price, link, ISBN, rental, bookdescription, photo):  # 나중에 이미지 이름도 인자로 받아와야함!
    ...
    # 이미지 넣는 곳
    image = Image.open("GIF/"+photo)
    
    canvas=Canvas(frame1,height=200,width=200)
    image = image.resize((150,210))
    photo1=ImageTk.PhotoImage(image)
    item4 = canvas.create_image(100,100,image = photo1)
    canvas.grid(row=0, column=3, columnspan=1, rowspan=6, sticky=E, padx=5)
     
    '''
    img=ImageTk.PhotoImage(file='GIF/' + photo, master=win1) # 나중에 이미지이름 인자로 받아와야함!
